...
class CardStreamController {
  constructor() {
    this.cardContainer = document.getElementById("cardStream");
    this.cardLine = document.getElementById("cardLine");
    this.speedIndicator = document.getElementById("speedValue");
    this.configRoot = document.querySelector(".section-3") || document.body;
  ...
  readConfigFromCSS() {
    const styles = window.getComputedStyle(this.configRoot);
    ...
    this.cardWidth = readValue("--card-w", this.cardWidth);
    this.cardHeight = readValue("--card-h", this.cardHeight);
    this.cardGap = readValue("--card-gap", this.cardGap);
  }

  calculateDimensions() {
    this.cardContainerWidth = this.cardContainer.offsetWidth;
  ...
  setupEventListeners() {
    ...
    this.cardLine.addEventListener("wheel", (e) => this.onWheel(e), { passive: true });
  ...
  updateCardPosition() {
    const cardContainerWidth = this.cardContainerWidth;
    const cardLineWidth = this.cardLineWidth;

    if (this.position < -cardLineWidth) {
      this.position = cardContainerWidth;
    } else if (this.position > cardContainerWidth) {
  ...
  resetPosition() {
    this.position = this.cardContainerWidth;
    this.velocity = 120;
    this.direction = -1;
    this.isAnimating = true;
    this.isDragging = false;
  ...
  createCardWrapper(index) {
    ...
    const { width, height, fontSize, lineHeight } =
      this.calculateCodeDimensions(this.cardWidth, this.cardHeight);
  ...
  updateCardClipping() {
    ...
    // 获取当前卡片容器的变换偏移量（如果有的话，用于精确对齐扫描线）
...
class ParticleSystem {
  ...
  readCanvasHeight() {
    const cssH = parseFloat(window.getComputedStyle(this.canvas).height);
    return Number.isFinite(cssH) && cssH > 0 ? cssH : 250;
  }

  init() {
    this.scene = new THREE.Scene();
    this.h = this.readCanvasHeight();
  ...
  createParticles() {
    ...
    for (let i = 0; i < this.particleCount; i++) {
      ...
      positions[i * 3 + 1] = (Math.random() - 0.5) * this.h;
      positions[i * 3 + 2] = 0;
...
class ParticleScanner {
  constructor() {
    ...
    const cssH = parseFloat(window.getComputedStyle(this.canvas).height);
    this.h = Number.isFinite(cssH) && cssH > 0 ? cssH : 300;
    this.particles = [];
    this.count = 0;
  ...
  onResize() {
    ...
    this.lightBarX = this.w / 2;
    const cssH = parseFloat(window.getComputedStyle(this.canvas).height);
    this.h = Number.isFinite(cssH) && cssH > 0 ? cssH : this.h;
    this.setupCanvas();
  }
